<aside
    class="fixed left-0 top-16 h-[calc(100vh-64px)] w-16 lg:w-64 bg-white border-r shadow-sm transition-all duration-300 z-40">
        ...
        <nav class="flex flex-col space-y-2 text-gray-700">
           ...
           {{ request()->routeIs('verification') ? 'bg-indigo-100 text-indigo-600 font-semibold' : 'hover:bg-indigo-50 hover:text-indigo-600' }}">
            ...
            <a href="{{ route('chatify') }}"
                class="flex items-center gap-3 px-3 py-2 rounded-md transition-all duration-200 justify-center lg:justify-start
           {{ request()->routeIs(patterns: 'chatify') ? 'bg-indigo-100 text-indigo-600 font-semibold' : 'hover:bg-indigo-50 hover:text-indigo-600' }}">
            ...
            <a href="{{ route('admin.campaigns.index') }}"
                class="flex items-center gap-3 px-3 py-2 rounded-md transition-all duration-200 justify-center lg:justify-start
        {{ request()->routeIs('admin.campaigns.index') ? 'bg-indigo-100 text-indigo-600 font-semibold' : 'hover:bg-indigo-50 hover:text-indigo-600' }}">
                <i class="fa-solid fa-clipboard-check text-lg"></i>
                <span class="text-sm font-medium hidden lg:block">Verifikasi Campaign</span>
            </a>

            <a href="{{ route('admin.user.index') }}"
                class="flex items-center gap-3 px-3 py-2 rounded-md transition-all duration-200 justify-center lg:justify-start
        {{ request()->routeIs('admin.user.index') ? 'bg-indigo-100 text-indigo-600 font-semibold' : 'hover:bg-indigo-50 hover:text-indigo-600' }}">
                <i class="fa-solid fa-id-card text-lg"></i>
                <span class="text-sm font-medium hidden lg:block">Verifikasi Identitas User</span>
            </a>
            @endif
