<nav class=" fixed top-0 left-0 right-0 z-50 bg-custom-orange">
      ...
      <div class="flex h-16 items-center justify-between">
        ...
        <div class="hidden md:flex flex-1 justify-center">
          <div class="flex items-baseline space-x-4">
            ...
            <x-navlinks href="/donasi" :active="request()->is('donasi')">Donasi</x-navlinks>
            <x-navlinks href="/galang" :active="request()->is('galang')">Galang Bantuan</x-navlinks>
          ...
          <div class="ml-4 flex items-center md:ml-6">
            ...
            <div class="relative ml-3">
              ...
              <div id="user-menu" class="hidden absolute right-0 z-10 mt-2 w-48 origin-top-right rounded-md bg-white py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none" role="menu" aria-orientation="vertical" aria-labelledby="user-menu-button" tabindex="-1">
                @auth
                    <a href="{{ route('profile.show') }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100" role="menuitem" tabindex="-1" id="user-menu-item-0">Edit Profile</a>
                ...
                @else
                    <a href="{{ route('login') }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100" role="menuitem" tabindex="-1">Login</a>
        ...
        <div class="-mr-2 flex md:hidden">
          <!-- Mobile menu button -->
          <button type="button" class="relative inline-flex items-center justify-center rounded-md bg-gray-800 p-2 text-gray-400 hover:bg-gray-700 hover:text-white focus:outline-none focus:ring-2 focus:ring-white focus:ring-offset-2 focus:ring-offset-gray-800" onclick="toggleMobileMenu()" aria-controls="mobile-menu" aria-expanded="false">
    ...
    <div class="md:hidden" id="mobile-menu">
      <div class="space-y-1 px-2 pb-3 pt-2 sm:px-3">
        <a href="/" class="block rounded-md px-3 py-2 text-base font-medium {{ request()->is('/') ? 'bg-gray-900 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white' }}" aria-current="page">Beranda</a>
        
        <a href="/donasi" class="block rounded-md px-3 py-2 text-base font-medium {{ request()->is('donasi') ? 'bg-gray-900 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white' }}">Donasi</a>
        
        <a href="/galang" class="block rounded-md px-3 py-2 text-base font-medium {{ request()->is('galang') ? 'bg-gray-900 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white' }}">Galang Bantuan</a>
        
        <a href="/about" class="block rounded-md px-3 py-2 text-base font-medium {{ request()->is('about') ? 'bg-gray-900 text-white' : 'text-gray-300 hover:bg-gray-700 hover:text-white' }}">Tentang Kami</a>
      </div>
...
<img class="h-10 w-10 rounded-full"
  ...
  alt="{{ Auth::check() ? Auth::user()->name : 'Anonymous' }}">
          ...
          <div class="ml-3">
            <div class="text-base font-medium leading-none text-white">{{ Auth::check() ? $nama : 'Anonymous' }}</div><!-- Nama User ambil dari database -->
            <div class="text-sm font-medium leading-none text-gray-400">{{ Auth::check() ? $email : 'anonymous@example.com' }}</div>
          </div>
        ...
        <div class="mt-3 space-y-1 px-2">
          @auth
              <a href="{{ route('profile.show') }}" class="block rounded-md px-3 py-2 text-base font-medium text-gray-400 hover:bg-gray-700 hover:text-white">Edit Profile</a>
