<x-app-layout>
    <div class="max-w-3xl mx-auto py-10 px-6 bg-white rounded-2xl shadow-xl">
        <h1 class="text-3xl font-bold mb-8 text-center text-amber-600">Donasi untuk: <span class="text-gray-800">{{ $campaign->title }}</span></h1>
        ...
        <form action="{{ route('donations.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            {{-- Nama Donatur --}}
            <div>
                <label class="block text-gray-700 font-medium mb-2"><i class="fas fa-user mr-1"></i> Nama Donatur</label>
                ...
                @if (auth()->check())
                    <div class="mt-2 flex items-center">
                        <div class="mx-2">
                        <input type="checkbox" id="hide_name" name="hide_name" class="mr-2 rounded-full text-amber-500 focus:ring-2 focus:ring-amber-400" {{ old('hide_name') ? 'checked' : '' }}>
                        <label for="hide_name" class="text-sm text-gray-600">Sembunyikan Nama (Donasi Anonim)</label>
                        </div>
                    </div>
                @endif
            </div>
            ...
            @if ($campaign->type === 'financial')
                <div>
                    <label class="block text-gray-700 font-medium mb-2"> <i class="fas fa-coins mr-1"></i> Jumlah Donasi (Rp)</label>
            ...
            @elseif ($campaign->type === 'goods')
                <div>
                    <label class="block text-gray-700 font-medium mb-2"><i class="fas fa-box mr-1"></i> Deskripsi Barang</label>
                ...
                <div>
                    <label class="block text-gray-700 font-medium mb-2"> <i class="fas fa-sort-numeric-up mr-1"></i> Jumlah Barang</label>
                    <input type="number" name="item_quantity" value="{{ old('item_quantity') }}" min="1" required
                        class="w-full border border-gray-300 rounded-lg px-4 py-2 focus:ring-2 focus:ring-amber-400">
                    @error('item_quantity')
                        <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                    @enderror
            ...
            @elseif ($campaign->type === 'emotional')
                <div>
                    <label class="block text-gray-700 font-medium mb-2"><i class="fas fa-hand-holding-heart mr-1"></i> Jumlah Sesi Dukungan</label>
            ...
            <div>
                ...
                <i class="fa-regular fa-image"></i></i> Bukti Pembayaran 
                </label>
